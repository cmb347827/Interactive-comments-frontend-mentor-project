...
function getGeneratedHTML(insert,which){
    const addMargin = (elementsData.nested)?'addMargin':'';
  ...
  //see lines 188/189 , 232/234, 267/270
...
function displayMessages(){
    ...
    elementsData.newJsonData.comments.forEach((comment,index)=>{
         ...
         if(comment.replies.length>0){
            ...
            comment.replies.forEach(reply=>{
              //check to see who the reply is from.
          ...
          if(index===(elementsData.newJsonData.comments.length-1)) {
            //currentUsers new comment or last comment at bottom, with the 'send' button.
            elementsData.nested=false;
    ...
    //new comments have been added, check the screen width and setAria for either mobile or desktop buttons.
...
const setCommentScore=(parent,val)=>{
                                                                            // needs IDs at lines => 154, 173 + 207, 244
...
//Below function is used to generate new comments and new replies by the currentuser , called in addButtonEvents()
...
function addButtonsEvents(){    
  ...
  //upvote/downvote apply to both currentuser comments and other user comments.       // needs IDs at lines => 154, 173 + 207, 244
  ...
  document.querySelectorAll('.js-submit-reply').forEach((btn,btnIndex)=>{
      btn.addEventListener('click',(event)=>{
        ...
        const content = event.target.previousElementSibling.value;
        const inReplyToThisComment= event.target.parentElement.parentElement.previousElementSibling;          //id at line 150
  ...
  //gets all currentUser textarea elements (needed for edit and delete actions)
  ...
  //get all cancel edit buttons from the currentUser comments   line 239
  ...
  //get all update buttons from the currentuser comments, see line 238 , is from hidden form (mobile or desktop makes no difference)
    ...
    EditNodes.forEach((btn,index)=>{
      ...
      btn.addEventListener("click", ()=>{
        ...
        if(!textareaNodes[index].classList.contains('edit-border')){
            //the textarea border is not visible, so add the class
            textareaNodes[index].classList.add('edit-border'); 
        }else if(textareaNodes[index].classList.contains('edit-border')){
             //the border is visible, this happens when the edit button is double clicked, typically the user changing their mind, remove the border.
      ...
      (jsupdateNodes[index]).addEventListener('click',(event)=>{
          ...
          id = Number(textareaNodes[index].closest('div').id);              //id at line 231  Ids at line 189,234,and 270(textareas) is purely for the associated label.
  ...
  deleteNodes.forEach((btn,index)=>{
    btn.addEventListener("click", (event)=>{
      ...
      //pop-up dialog asks to make sure to delete                                                        //ids at lines 219 and 250 for mobile/desktop
